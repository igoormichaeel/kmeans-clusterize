...
/*
 * Iterate through all data points and calculate sum of data points of columns
 * and get count for the given cluster. This will be used later to compute mean
 * and update the centroid.
 */
#pragma omp parallel for num_threads(threads) \
    reduction(+ : cluster_count, sum_cluster_columns)
    for (int row = 0; row < model->rows; row++) {
        for (int k = 0; k < model->no_clusters; k++) {
            if (model->data_clusters[row] == k) {
                ...
                // It will be used later to update the centroid or re-compute
                // the centroid.
                for (int i = 0; i < model->columns; i++) {
                    sum_cluster_columns[k * model->columns + i] +=
