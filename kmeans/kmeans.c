...
static void update_r_threaded(kmeans_config *config) {
    ...
    if (num_threads < 1) {
        ...
        for (i = 0; i < num_threads; i++) {
             ...
             * offest to the start of the batch the thread is responsible for,
             * and the object count number adjusted similarly.
             */
            ...
            if (i == num_threads - 1) {
                thread_config[i].num_objs +=
                    config->num_objs - num_threads * obs_per_thread;
            }
            ...
            rc =
                pthread_create(&thread[i], &thread_attr, update_r_threaded_main,
...
static void *update_means_threaded_main(void *arg) {
...
static void update_means_threaded(kmeans_config *config) {
    /* We only spin up threading infra if we need more than one core */
    /* running. We keep the threshold high so the overhead of */
    /* thread management is small compared to thread compute time */
    ...
    if (num_threads < 1) {
        ...
        for (i = 0; i < num_threads; i++) {
            /* Now we just run the thread, on its subset of the data */
            rc = pthread_create(&thread[i], &thread_attr,
                                update_means_threaded_main, (void *)config);
            if (rc) {
                printf("ERROR: return code from pthread_create() is %d\n", rc);
                exit(-1);
            }
...
kmeans_result kmeans(kmeans_config *config) {
    ...
    if (!config->max_iterations) config->max_iterations = KMEANS_MAX_ITERATIONS;
