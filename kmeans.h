 ...
 * @param cluster the cluster number we are actually generating a centroid for
 * here
 * @param centroid the object to write the centroid result into (already
 * allocated)
 */
typedef void (*kmeans_centroid_method)(const Pointer *objs, const int *clusters,
                                       size_t num_objs, int cluster,
                                       Pointer centroid);
